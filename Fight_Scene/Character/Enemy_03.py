...
class WizardQ:
    def __init__(self, x, y, flip, data, sprite_sheet, animation_steps, count):
        ...
        self.action = (
            0  # 0:idle 1:walk 2:jump 3:jump-up 4:hit 5:jump 6:disappear 7:die 8:wink
        )
        self.frame_index = 0
    ...
    def move(self, SCREEN_WIDTH, SCREEN_HEIGHT, surface, target):
        ...
        if (
            ...
            self.action = random.choice([2, 3, 5, 7])
            while cooldown > 0:
                cooldown -= 1
            if self.action == 2:
                self.running == True
                ...
                if self.rect.x > target.rect.x:
                    self.rect.x -= 10 * (random.randint(10, 16))
                else:
                    self.rect.x += 10 * (random.randint(10, 16))
            elif self.action == 7:
                self.attack_type = 2
                ...
                self.attacking = True
                if self.rect.x > target.rect.x:
                    self.rect.x -= 10 * (random.randint(8, 14))
                else:
                    self.rect.x += 10 * (random.randint(8, 14))
        elif self.hit == True:
            self.rect.x += 5
        # 0:idle 1:walk 2:run 3:jump-up 4:hit 5:disappear 6:die 7:wink
        if self.rect.x == target.rect.x:
            self.rect.x += 5
    ...
    def update(self, target):
        if self.blood <= 0:
            ...
            if self.attack_type == 1:
                self.update_action(5)  # disappear
            elif self.attack_type == 2:
                self.update_action(7)  # wink
        elif self.jump == True:
        ...
        if self.frame_index >= len(self.animation_list[self.action]):
            ...
            if self.alive == False:
                self.frame_index = len(self.animation_list[6]) - 1  # 死了就停在最後一個畫面
            else:
                self.frame_index = 0
